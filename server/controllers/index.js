...
const createDog = async (req, res) => {
  ...
  if (!req.body.name || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'Missing one or more required parameter(s)' });
  }
  ...
  try {
    ...
    console.log(e);
    return res.status(500).json({ error: 'Something went wrong while creating the dog' });
  }
};
...
const findDog = async (req, res) => {
  ...
  let doc;
  try {
    doc = await Dog.findOneAndUpdate(req.query, { $inc: { age: 1 } }, { new: true }).exec();
  } catch (e) {
    console.log(e);
    return res.status(500).json({ error: 'Something went wrong' });
  }
